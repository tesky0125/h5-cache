...
class AbstractStore {
   ...
   * @return {object}
   */
  buildStorageEntity(value, expiretime, savetime) {
    const entity = {
      value: value,
  ...
  set(key, value, expiretime, savetime) {
    ...
    // 将key和过期时间放到缓存中
    this.setStoredKeys(key, myExpiretime);
    const entity = this.buildStorageEntity(value, myExpiretime, mySavetime);
    try {
